...
TEST(Mat44UtilsTest, GeneralInverse)
{
    const m44f m(1,2,3,4,
            0,7,6,8,
            0,0,12,11,
            0,0,0,16);

    const m44f inv = GeneralInverse(m);
    const m44f result = inv * m;

    EXPECT_LT(NormDiffSq(m44f(m44f::identity), result), 1e-3f);
}
